...
// Import the necessary interfaces from your central types file
...
    if (!user) { // If user is still null, try to create one
      try {
        user = await User.create({
          name,
          email: email || '',
...
    }
    ...
    // After the 'if (!user)' block, 'user' is guaranteed to be an IUser document
    // because either it was found, or it was created.
    // We can confidently use the non-null assertion operator here.
    const alreadyInGroup = groupMembers.some(m =>
      m.user.toString() === user!._id.toString() // FIX: Add '!' here
    );
    
    if (alreadyInGroup) {
      return NextResponse.json({ message: "Member already in the group." }, { status: 400 });
...
    group.members.push({
      user: user!._id, // FIX: Also add '!' here if user might still be null, though it shouldn't be
      amount: 0,
      joinedAt: new Date(),
...
    await group.save();
    
    await User.findByIdAndUpdate(user!._id, { // FIX: Also add '!' here
      $addToSet: { groups: group._id }
    });
...
      member: {
        user: {
          _id: user!._id, // FIX: Also add '!' here
          name: user!.name, // FIX: Also add '!' here
          email: user!.email // FIX: Also add '!' here
        },
        amount: 0,
        joinedAt: new Date()
      }
