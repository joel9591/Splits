// // I:\New folder-Splits\app\api\generate-trip\route.ts
...
// import { TripPlan, ITrip } from "@/lib/types";
...
//         // Replace Rupee symbol with "Rs." or "INR " in the parsed data if it somehow made it through
...
//     y -= 10;
//     y = drawText(tripPlan.estimatedCost, font, 12, 60, y); // This line is where the problem originates
...
import { TripPlan, ITripDocument, ApiResponse } from "@/lib/types"; // Import ITripDocument and ApiResponse
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const newTrip = new Trip({
        ...tripPlan,
        createdBy: user._id, // user._id is Types.ObjectId
        pdfUrl: pdfUrl,
    });
    
    await newTrip.save();

    // Prepare the response to match ApiResponse
    return NextResponse.json({
        tripPlan: tripPlan,
        pdfId: pdfId,
        pdfUrl: pdfUrl,
        _id: newTrip._id.toString(), // Convert ObjectId to string for frontend
        createdBy: newTrip.createdBy.toString(), // Convert ObjectId to string for frontend
        createdAt: newTrip.createdAt.toISOString(), // Convert Date to ISO string for frontend
        updatedAt: newTrip.updatedAt.toISOString(), // Convert Date to ISO string for frontend
    } as ApiResponse); // Explicitly cast to ApiResponse to ensure type safety
    // You can remove the explicit cast if the properties are an exact match
    // and TypeScript can infer it. However, it's good practice for clarity.
